...
import 'GlobalHistogramBinarizer.dart';
...
class HybridBinarizer extends GlobalHistogramBinarizer {
  ...
  BitMatrix getBlackMatrix() {
    ...
    if (width >= _MINIMUM_DIMENSION && height >= _MINIMUM_DIMENSION) {
      Uint8ClampedList luminances = source.getMatrix();
      ...
      List<List<int>> blackPoints = _calculateBlackPoints(luminances, subWidth, subHeight, width, height);

      BitMatrix newMatrix = new BitMatrix(width, height);
      _calculateThresholdForBlock(luminances, subWidth, subHeight, width, height, blackPoints, newMatrix);
      _matrix = newMatrix;
    } else {
   ...
   */
  static void _calculateThresholdForBlock(Uint8ClampedList luminances,
                                                 int subWidth,
                                                 int subHeight,
                                                 int width,
                                                 int height,
   ...
   */
  static void _thresholdBlock(Uint8ClampedList luminances,
                                     int xoffset,
                                     int yoffset,
                                     int threshold,
                                     int stride,
                                     BitMatrix matrix) {
    for (int y = 0, offset = yoffset * stride + xoffset; y < _BLOCK_SIZE; y++, offset += stride) {
   ...
   */
  static List<List<int>> _calculateBlackPoints(Uint8ClampedList luminances,
                                              int subWidth,
                                              int subHeight,
                                              int width,
                                              int height) {
    int maxYOffset = height - _BLOCK_SIZE;
    int maxXOffset = width - _BLOCK_SIZE;
    List<List<int>> blackPoints = List.generate(subHeight, (_)=> new List(subWidth));
    for (int y = 0; y < subHeight; y++) {
      int yoffset = y << _BLOCK_SIZE_POWER;
      if (yoffset > maxYOffset) {
        yoffset = maxYOffset;
      ...
      for (int x = 0; x < subWidth; x++) {
        ...
        int min = 0xFF;
        int max = 0;
        for (int yy = 0, offset = yoffset * width + xoffset; yy < _BLOCK_SIZE; yy++, offset += width) {
